...
export class App {
  ...
  private renderGate(profiles: ResearcherProfile[], message = ''): void {
    ...
    const header = el('header', 'credential-header');
    const headerText = el('div', 'credential-title-group');
    headerText.append(el('h1', '', 'SCP FOUNDATION'), el('p', 'eyebrow', 'SECURE RESEARCH NETWORK / PERSONNEL TERMINAL'));
    const headerLogo = el('div', 'foundation-mark foundation-logo-mark');
    headerLogo.setAttribute('aria-hidden', 'true');
    header.append(headerText, headerLogo);
  ...
  private renderAuthentication(lines: string[], delay: number): Promise<void> {
    return new Promise((resolve) => {
      ...
      const header = el('header', 'credential-header');
      const headerText = el('div', 'credential-title-group');
      headerText.append(el('h1', '', 'CREDENTIAL AUTHENTICATION'), el('p', 'eyebrow', 'FOUNDATION SECURE RESEARCH NETWORK'));
      const headerLogo = el('div', 'foundation-mark foundation-logo-mark');
      headerLogo.setAttribute('aria-hidden', 'true');
      header.append(headerText, headerLogo);
      terminal.append(header);
