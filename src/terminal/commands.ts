...
export interface TerminalResult {
  output: string[];
  action?: { type: 'open-record'; id: string } | { type: 'show-view'; view: string } | { type: 'logout' };
}
...
export function executeCommand(command: TerminalCommand, profile: ResearcherProfile, archive: ArchiveIndexEntry[]): TerminalResult {
  ...
  switch (command.name) {
    case 'HELP': return { output: ['AVAILABLE: HELP, OPEN <record>, FIND <query>, LIST ASSIGNMENTS|MAIL, PROFILE, CLEARANCE, HISTORY, NOTES, BOOKMARKS, LOGOUT'] };
    case 'OPEN': {
      if (!argText) return { output: ['OPEN requires a record identifier.'] };
      const needle = argText.toLowerCase();
      const match = archive.find((entry) => entry.slug.toLowerCase() === needle || entry.id.toLowerCase() === needle || entry.title.toLowerCase() === needle);
    ...
    case 'HISTORY': return { output: profile.history.slice(-8).map((entry) => `${entry.at.slice(0, 19)} ${entry.detail}`) };
    case 'NOTES': return { output: [`${Object.values(profile.notes).flat().length} RESEARCH NOTE(S).`], action: { type: 'show-view', view: 'notes' } };
    case 'BOOKMARKS': return { output: profile.bookmarks.length ? profile.bookmarks : ['NO BOOKMARKS.'] };
